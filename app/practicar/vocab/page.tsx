...
const VOCAB_LESSONS = Object.keys(GENKI_VOCAB_BY_LESSON).map(Number).sort((a, b) => a - b);
const KANJI_LESSONS = Object.keys(GENKI_KANJI_BY_LESSON).map(Number).sort((a, b) => a - b);
...
function getOptions(correct: QuizItem, pool: QuizItem[]): string[] {
  ...
  const supplemented =
    others.length < 3
      ? [...new Set(toQuizItems(Object.values(GENKI_VOCAB_BY_LESSON).flat()).map((i) => i.es).filter((e) => e !== correctEs))]
      : others;
...
export default function VocabPage() {
  ...
  const [type, setType] = useState<"vocab" | "kanji">("vocab");
  ...
  useEffect(() => {
    setLastActivity("Vocab y Kanji", "/practicar/vocab");
  }, []);
