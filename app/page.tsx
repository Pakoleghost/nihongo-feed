...
export default function HomePage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Solo cargamos contenido si es Admin o Alumno aprobado
  ...
  // Pantalla de Bloqueo para alumnos no aprobados
  if (!loading && myProfile && !myProfile.is_approved && !myProfile.is_admin) {
    return (
      <div style={{ textAlign: "center", padding: "100px 20px", fontFamily: "sans-serif" }}>
        ...
        <h2 style={{ fontSize: "24px", margin: "20px 0 10px" }}>¡Hola, {myProfile.username}!</h2>
        <p style={{ color: "#666", maxWidth: "400px", margin: "0 auto", lineHeight: "1.5" }}>
          Tu cuenta ha sido registrada correctamente, pero está en espera de aprobación. 
          Te avisaremos en cuanto el administrador te asigne a un grupo.
        </p>
        <button 
          onClick={() => supabase.auth.signOut().then(() => router.push("/login"))} 
          style={{ marginTop: "30px", background: "none", border: "1px solid #ddd", padding: "10px 20px", borderRadius: "20px", cursor: "pointer", color: "#888" }}
        >
          Cerrar sesión
        </button>
      </div>
      ...
      <header style={{ 
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          ...
          <button 
            ...
            style={{ 
              background: showArchived ? "#f0f0f0" : "none", border: "none", fontSize: "12px", 
              color: showArchived ? "#2cb696" : "#888", padding: "5px 10px", borderRadius: "10px", cursor: "pointer"
            }}
          >
              ...
              <div style={{ paddingTop: "10px" }}>
                {teacherDirectives.filter(p => !dismissedAnnouncements.includes(p.id)).map(post => (
                  <div key={post.id} style={{ 
                    ...
                    }} style={{ position: "absolute", top: "10px", right: "10px", background: "none", border: "none", color: "#888", cursor: "pointer" }}>✕</button>
